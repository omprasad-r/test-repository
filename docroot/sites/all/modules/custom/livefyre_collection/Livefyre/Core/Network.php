...
class Network extends Core {
	...
	public function buildUserAuthToken($userId, $displayName, $expires) {
		...
		$token = array("domain" => $this -> getData() -> getName(), "user_id" => $userId, "display_name" => $displayName, "expires" => time() + (int)$expires);
	...
	public function validateLivefyreToken($lfToken) {
		...
		return $tokenAttributes -> domain == $this -> getData() -> getName() && $tokenAttributes -> user_id == self::DEFAULT_USER && $tokenAttributes -> expires >= time();
	}
