...
/**
 * Defines the Drupal root directory as the acsf_events directory.
 */
...
class UnitTest extends PHPUnit_Framework_TestCase {
  ...
  public function setUp() {
    $files = array(
      ...
      __DIR__ . '/../classes/AcsfEvent.inc',
      __DIR__ . '/../classes/AcsfEventDispatcher.inc',
      __DIR__ . '/../classes/AcsfEventHandler.inc',
  ...
  public function testAcsfEventHandlersEmpty() {
    $event = new AcsfEvent(new AcsfEventDispatcher(), new AcsfLog(), 'unit_test', array(), array());
  ...
  public function testAcsfEventExecute() {
    ...
    $event = new AcsfEvent(new AcsfEventDispatcher(), new AcsfLog(), 'unit_test', $registry, array());
    $event->run();
    $debug = $event->debug();
  ...
  public function testAcsfEventCreate() {
    $event = AcsfEvent::create('unit_test', array());
    $event->run();
    $debug = $event->debug();
    $this->assertCount(3, $debug['handlers']['complete']);
  }
   ...
   *
   * @expectedException AcsfEventHandlerIncompatibleException
   */
