  ...
  */
  Drupal.behaviors.acquia = {
    attach: function (context, settings) {
      ...
      Drupal.behaviors.acquia.currentSizeBucket = '';
      Drupal.behaviors.acquia.enhanceLayout();
      $(window).resize(Drupal.behaviors.acquia.enhanceLayout);
  ...
  Drupal.behaviors.acquia.getBucketSize = function () {
    var bucket = '';
    var buckets = Drupal.behaviors.acquia.layoutSizeBuckets;
    var viewport = window.innerWidth;
    for (bucket in buckets) {
      if (viewport >= buckets[bucket]) {
        return bucket;
      }
    }
    return bucket;
  ...
  Drupal.behaviors.acquia.enhanceLayout = function () {
    var bucket = Drupal.behaviors.acquia.getBucketSize();
    if (bucket == Drupal.behaviors.acquia.currentSizeBucket) {
      return;
    }
    Drupal.behaviors.acquia.currentSizeBucket = bucket;
    switch (bucket) {
      case 'x_large' :
        $.each($('.sidebar.right'), function (index, item) {
          $this = $(this);
          $parent = $this.parent('.box');
          $this.prependTo($parent);
        });
        $.each($('.sidebar.left'), function (index, item) {
          $this = $(this);
          $parent = $this.parent('.box');
          $this.prependTo($parent);
        });
