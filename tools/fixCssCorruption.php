...
/**
 * This script determines whether css corruption from the pseudoclass sorting
 * bug exists in the customized css files within a theme, and if so the
 * corruption is removed and a line printed that describes the corrupted
 * theme.  Related bugfix: AN-16013.
 */
if (count($argv) != 2) {
...
/**
 * Detects and fixes css corruption on custom css files.
 *
 * @param {String} $path
 *   The path to the theme.
 * @return
 *   A boolean that indicates whether the theme had css corruption.  true
 *   means it had corruption; false means it didn't'.
 */
function fixCssCorruption($path) {
  $result = false;
  $result |= fixCssCorruptionInFile("$path/custom.css");
  $result |= fixCssCorruptionInFile("$path/palette.css");
  return $result;
}

/**
 * Fixes css corruption in the specified file.
 *
 * @param {String} $cssFile
 *   The path to the css file to check.
 *
 * @return
 *   A boolean that indicates whether the file had css corruption.  true means
 *   it had corruption; false means it didn't'.
 */
function fixCssCorruptionInFile($cssFile) {
  // The css corruption caused by the pseudoclass sorting error has the
  // following pattern:
  // [one character] {
  // }
  // Note that the rule is empty.  That makes it safe to delete any rule in
  // custom.css or palette.css that matches the pattern.
  $contents = file_get_contents($cssFile);
  $replacedContents = $contents;
  $replacedContents = preg_replace('/^(.){1} \{\s*\}$/m', '', $contents);
  if ($contents !== $replacedContents) {
    file_put_contents($cssFile, $replacedContents);
    return true;
  }
  return false;
}
