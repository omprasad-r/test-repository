...
var gardenerBase = "http://qatest" + userNum + "site.gsteamerperf.acquia-sites.com/";
selenium.setTimeout(120000);
selenium.open(gardenerBase);
Gardens.login(gardenerBase, "qatestuser2", "qatestuser");
try {
    Themer.startThemeBuilder();
    // Next, begin the transaction. This tells BrowserMob to start recording timings
    for (var i = 0; i < loops; i++) {
        browserMob.log("Starting iteration: " + i);
        browserMob.beginTransaction();
        browserMob.beginStep("Save/Overwrite Theme");
        try {
            Themer.saveThemeAs('testThemeSave');
        } 
        catch (themerException) {
            browserMob.log("Save as test failed with exception: " + themerException);
            throw "Save theme as test failed with exception: " + themerException;
        }
        browserMob.endStep();
    ...
    for (var i = 0; i < loops; i++) {
        browserMob.log("Starting iteration: " + i);
        browserMob.beginTransaction();
        browserMob.beginStep("Publish Theme");
        try {
            Themer.publishTheme('testThemePublish');
        } 
        catch (themerException) {
            browserMob.log("Publish test failed with exception: " + themerException);
            throw "Publish theme test failed with exception: " + themerException;
            
        }
        browserMob.endStep();
        browserMob.endTransaction();
    }
    browserMob.log("Exiting TB normally");
    Themer.exitThemeBuilder();
} 
catch (e) {
    try {
        browserMob.log("Exiting TB through an exception: " + e);
        Themer.exitThemeBuilder();
    } 
    catch (f) {
        browserMob.log("OMG: Even Exiting TB through an exception threw one:" + f);
        throw f;
    }
    throw e;
}